...
float p(vec2 st, float y, float off) {
    float t = step(0.172 - y,1.-st.y);
    float b = step(0.8 + y, st.y);
    float l = step(0.7 + (sin(u_time + off) + off), st.x);
    float r = step(0.2 - (sin(u_time + off) + off), 1.-st.x);
    return t*b*l*r;
}
...
void main() {
    ...
    vec3 p1 = vec3(p(fPos,0.036,random(iPos + u_time)));
    vec3 p2 = vec3(p(fPos,-0.1,random(iPos ))) * cloud;
    vec3 p3 = vec3(p(fPos,-0.5,random(0.3))) * blue;
    vec3 p4 = vec3(p(fPos,-0.512,0.3)) * vec3(1.,1.,0.);
